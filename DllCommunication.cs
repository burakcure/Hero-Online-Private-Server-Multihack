...
namespace OSPE
{
    ...
    static class DllCommunication
    {
        ...
        */
        ...
        public static void WriteCommandToCmdMMF(ServerCodes sc, byte[] data = null, UInt16 newLength = 0, ushort socketId = 0,int echo=0)
        {
            ...
            {
                try { cmdMMF = new SharedMemory.CircularBuffer(_cmdMmfName); }
                catch {
                    cmdMMF.Close();
                    CircularBuffer circularBuffer = new SharedMemory.CircularBuffer(_cmdMmfName);
                    cmdMMF = circularBuffer;
